...
app.on('window-all-closed', () => {{ // Ensuring that the application closes when the close button is selected.
    app.quit();
}
});
...
const mainMenuTemplate = [
	{
		label: "Clicker Game (" + process.env.user || process.env.username + ")",
		submenu: [
			{
